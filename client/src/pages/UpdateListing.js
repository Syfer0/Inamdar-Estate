...
export default function CreateListing() {
    ...
    useEffect(() => {
        const fetchListing = () => __awaiter(this, void 0, void 0, function* () {
            const listingId = params.listingId;
            const res = yield fetch(`/api/listing/get/${listingId}`);
    ...
    const handleImageSubmit = (e) => {
        if (files.length > 0 && files.length + formData.imageUrls.length < 7) {
            ...
            Promise.all(promises)
                .then((urls) => {
                setFormData(Object.assign(Object.assign({}, formData), { imageUrls: formData.imageUrls.concat(urls) }));
                setImageUploadError(false);
                setUploading(false);
            })
                .catch((err) => {
    ...
    const storeImage = (file) => __awaiter(this, void 0, void 0, function* () {
    ...
    const handleRemoveImage = (index) => {
        setFormData(Object.assign(Object.assign({}, formData), { imageUrls: formData.imageUrls.filter((_, i) => i !== index) }));
    };
    const handleChange = (e) => {
        if (e.target.id === 'sale' || e.target.id === 'rent') {
            setFormData(Object.assign(Object.assign({}, formData), { type: e.target.id }));
        }
        if (e.target.id === 'parking' ||
            e.target.id === 'furnished' ||
            e.target.id === 'offer') {
            setFormData(Object.assign(Object.assign({}, formData), { [e.target.id]: e.target.checked }));
        }
    ...
    const handleSubmit = (e) => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            setLoading(true);
            setError(false);
            const res = yield fetch(`/api/listing/update/${params.listingId}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify(Object.assign(Object.assign({}, formData), { userRef: currentUser._id })),
            });
            const data = yield res.json();
            setLoading(false);
            if (data.success === false) {
      ...
      <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row gap-4'>
        ...
        <div className='flex flex-col flex-1 gap-4'>
          ...
          {formData.imageUrls.length > 0 &&
            formData.imageUrls.map((url, index) => (<div key={url} className='flex justify-between p-3 border items-center'>
