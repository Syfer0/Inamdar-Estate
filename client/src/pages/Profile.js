...
export default function Profile() {
    ...
    const handleFileUpload = (file) => {
        ...
        uploadTask.on('state_changed', (snapshot) => {
            ...
            getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => setFormData(Object.assign(Object.assign({}, formData), { avatar: downloadURL })));
        });
    };
    ...
    const handleSubmit = (e) => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const res = yield fetch(`/api/user/update/${currentUser._id}`, {
                method: 'POST',
                headers: {
    ...
    const handleDeleteUser = () => __awaiter(this, void 0, void 0, function* () {
        try {
            dispatch(deleteUserStart());
            const res = yield fetch(`/api/user/delete/${currentUser._id}`, {
                method: 'DELETE',
            });
            const data = yield res.json();
            if (data.success === false) {
    ...
    const handleSignOut = () => __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const res = yield fetch('/api/auth/signout');
            const data = yield res.json();
            if (data.success === false) {
                dispatch(deleteUserFailure(data.message));
                return;
    ...
    const handleShowListings = () => __awaiter(this, void 0, void 0, function* () {
        try {
            setShowListingsError(false);
            const res = yield fetch(`/api/user/listings/${currentUser._id}`);
            const data = yield res.json();
            if (data.success === false) {
    ...
    const handleListingDelete = (listingId) => __awaiter(this, void 0, void 0, function* () {
        try {
            const res = yield fetch(`/api/listing/delete/${listingId}`, {
                method: 'DELETE',
            });
            const data = yield res.json();
            if (data.success === false) {
      ...
      <form onSubmit={handleSubmit} className='flex flex-col gap-4'>
        ...
        <p className='text-sm self-center'>
          {fileUploadError ? (<span className='text-red-700'>
            ...
            </span>) : filePerc > 0 && filePerc < 100 ? (<span className='text-slate-700'>{`Uploading ${filePerc}%`}</span>) : filePerc === 100 ? (<span className='text-green-700'>Image successfully uploaded!</span>) : ('')}
        </p>
